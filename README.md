...
*   `src/app`: Корневой макет приложения, провайдеры и глобальные стили (Next.js App Router).
*   `src/screens`: Композиция фич и виджетов в полноценные страницы (аналогично слою `pages` в FSD).
*   `src/features`: Реализация бизнес-логики и сложных пользовательских взаимодействий.
*   `src/entities`: Определение предметной области и их базовое UI-представление.
*   `src/widgets`: Композитные UI-компоненты (например, шапка или футер).
*   `src/shared`: Переиспользуемые низкоуровневые утилиты, UI-компоненты-примитивы и API-хелперы.
*   `payload/`: Директория с конфигурацией Payload CMS (коллекции, блоки, конфиг), расположенная вне `src`.
